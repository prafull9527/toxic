...
void search(char *filename, char *pattern, int count);
...
int main() {
    ...
    while (1) {
        ...
        if (token_count > 2 && strcmp(tokens[0], "search") == 0) {
            ...
            int count = (token_count > 4 && strcmp(tokens[4], "c") == 0) ? 1 : 0;
            search(filename, pattern, count);
        } else {
...
void search(char *filename, char *pattern, int count) {
    ...
    if (count) {
        ...
        printf("Pattern '%s' found in file '%s': %s\n", pattern, filename, (occurrences > 0) ? "Yes" : "No");
    }
}
*/
